...
def calculate_positions(consolidated_analysis: Dict[str, Any], capital_per_position: float = 100.0, api_key: str = None) -> Dict[str, Any]:
    ...
    Args:
        ...
        api_key: Clé API Moralis (optionnel, utilise .env par défaut)
    ...
    if not api_key:
        raise ValueError("Clé API Moralis manquante. Renseignez MORALIS_API_KEY dans le fichier .env")
    ...
    for entry in tweets_analysis:
        ...
        position = {
            ...
            "entry_price": price,
            "block_number": block_number,  # Nouveau: numéro de bloc pour référence
